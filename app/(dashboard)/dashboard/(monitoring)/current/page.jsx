...
export default function Current() {
    ...
    try {
      ...
      if (data.message === "OK") {
        ...
        if (selectDev.length !== 0) {
          ...
          if (data.monitoring["data"]["datacurrents"][0] === null) {
            ...
            if (minutes >= 5) {
              ...
              setDataCurrent(data.monitoring["data"]["datacurrents"]);
              setSignal(true);
              setChannel("Stable");
              setOnLoading(false);
            }
  ...
  const { data, isLoading, error } = useSWR(
    ...
    async ([url, localTenant, locationid, start_date]) => {
      try {
        return await fetchCurrentRealtime(
          url,
          localTenant,
          locationid,
          start_date
