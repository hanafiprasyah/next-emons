...
export default function Grounding() {
  ...
  const fetchGroundRealtime = async (url, tenant, locationid) => {
    return fetch(url, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      ...
      .then((datas) => {
        if (datas) {
          selectDev.indexOf(0).length != 0
            ? setDataGround(datas.ground["data"])
            : setDataGround([]);
        } else {
  ...
  const { data, error } = useSWR(
    localTenant !== "" && localTenant !== undefined
      ? [
          "/api/monitoring/ground/getdata",
          localTenant,
          selectDev.length === 0 ? "0" : selectDev[0],
        ]
      : null,
    ([url, localTenant, locationid]) =>
      fetchGroundRealtime(url, localTenant, locationid),
    {
