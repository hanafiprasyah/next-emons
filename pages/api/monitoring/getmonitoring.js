...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      `${process.env.BASE_URL}:${process.env.GENERAL_PORT}/dashboard/getlastdata`,
      {
        method: "POST",
        headers: {
    ...
    if (data) {
      res.status(200).json({ message: response.statusText, monitoring: data });
    } else {
      res.status(500).json({ message: response.statusText, monitoring: null });
    }
  } catch (e) {
    if (process.env.NODE_ENV === "development") {
      console.error(e);
